...
export default function Page(props) {
    return (
        <RenderQS1Detail data={props.qs1} error={props.error}/>               
    );
}

function RenderQS1Detail(props) {
    const { 
        data,
        error,
    } = props;
        ...
        :
            <Form className="form">
                ...
                <Form.Group as={Row} controlId="formSsn">
                    <Form.Label column sm="5" className="field">SSN:</Form.Label>
                    <Col sm="7"><Form.Control plaintext readOnly defaultValue={data.ssn} /></Col>
                </Form.Group>
                ...
                <div className="table-title"><h5>E-mail</h5></div>   
                <RenderEmailTable data={data.emails}/>
            </Form>  
    );
}
...
function RenderEmailTable(props) {
    return (
        <Table bordered responsive size="sm">
            <thead>
                <tr>
            ...
            <tbody>
                <RenderEmailRows data={props.data}/>
            </tbody>
        </Table>
    );
}
function RenderEmailRows(props) {
    return props.data.map((item,index) => {
